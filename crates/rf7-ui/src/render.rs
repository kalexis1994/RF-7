...
pub fn header(state: &State) -> String {
    ...
    match &state.draft {
        Some(draft) => {
            ...
            let dirty = if draft.dirty { " lit" } else { "" };
            let _ = write!(
                out,
                "<button type=\"button\" class=\"key wide{dirty}\" data-action=\"save\"><span class=\"led\"></span>SAVE</button>"
            );
            out.push_str(
                "<button type=\"button\" class=\"key wide\" data-action=\"cancel\"><span class=\"led\"></span>EXIT</button>",
            );
        }
        None => {
            ...
            let _ = write!(
                out,
                "<button type=\"button\" class=\"key wide\" data-action=\"edit\"{}><span class=\"led\"></span>EDIT</button>",
                if editable && state.connected {
                    ""
                } else {
                    " disabled"
                }
            ...
            let _ = write!(
                out,
                "<button type=\"button\" class=\"key wide\" data-action=\"new\"{}><span class=\"led\"></span>NEW</button>",
                if state.connected { "" } else { " disabled" }
            );
        }
    }
    out.push_str("</div>");
...
fn display(state: &State) -> String {
    let (number, name) = match (&state.draft, state.selected_sound()) {
        (Some(draft), _) => (
            if draft.original_program_id.is_some() {
                "ED".to_owned()
            } else {
                "--".to_owned()
            },
...
/// `program-004` reads as 04 on the display; a saved program has no number.
fn program_number(id: &str) -> String {
    id.strip_prefix("program-")
        .and_then(|digits| digits.parse::<usize>().ok())
        .map(|slot| format!("{slot:02}"))
...
fn operator(state: &State, draft: &Draft, n: usize, carrier: bool) -> String {
    ...
    let mut out = format!(
        "<section class=\"{class}\" data-op=\"{n}\"><header><span class=\"badge\">OP{n}</span><span class=\"role\">{role}</span><span class=\"freq\">{}</span><span class=\"out\">{}</span></header><div class=\"screen\">{}</div>",
        esc(&frequency),
        integer(state, &f("out")),
...
mod tests {
    ...
    fn the_display_reads_like_the_instrument_it_is() {
        ...
        assert!(html.contains("<span class=\"code\">ED</span>"));
        ...
        assert!(html.contains("data-action=\"edit\""));
        assert!(!html.contains("data-action=\"edit\" disabled"));
        state.disconnect("Host timed out");
        assert!(header(&state).contains("HOST TIMED OUT"));
        assert!(header(&state).contains("data-action=\"edit\" disabled"));
    }
    ...
    fn the_voice_page_prints_the_chart_and_its_controls() {
        ...
        assert!(
            html.contains("data-field=\"feedback\" min=\"0\" max=\"7\" step=\"1\" value=\"6\"")
        );
        assert!(
    ...
    fn the_perform_page_follows_the_parameter_schema() {
        ...
        assert!(html.contains("data-param=\"0\" min=\"0\" max=\"2\" step=\"0.01\" value=\"0.3\""));
